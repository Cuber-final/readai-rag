...
from readai.core.exceptions import VectorStoreException, BookNotFoundException, BookNotProcessedException
...
class RAGEngine:
    ...
    async def process_book(self, book: Book) -> bool:
        ...
        try:
            ...
            # 创建索引
            index = self.vector_store_service.create_or_update_index(book.id, split_documents)
            
            logger.info(f"书籍处理完成: {book.id}, 总块数: {len(split_documents)}")
            return True
        except Exception as e:
            logger.error(f"处理书籍失败: {str(e)}")
            return False
...
"""
            ...
            # 进行流式对话
            return self.llm_service.chat_completion_stream(messages, model_name=model_name)
            
        except BookNotProcessedException as e:
            logger.error(f"书籍未处理: {e}")
            yield f"错误: 该书籍尚未处理为向量形式，请先处理该书籍。"
        except Exception as e:
            logger.error(f"与书籍聊天失败: {str(e)}")
            yield f"与书籍聊天失败: {str(e)}"
...
"""
    ...
    def _get_relevant_contexts(
        ...
        try:
            ...
            raise VectorStoreException(f"获取相关上下文失败: {str(e)}")
