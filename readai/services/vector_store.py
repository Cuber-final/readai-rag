...
class VectorStoreService:
    ...
    def _get_chroma_client(self, collection_name: str) -> chromadb.PersistentClient:
        ...
        try:
            ...
            logger.error(f"创建ChromaDB客户端失败: {str(e)}")
            raise VectorStoreException(f"创建ChromaDB客户端失败: {str(e)}")
    ...
    def _get_qdrant_client(self) -> QdrantClient:
        ...
        try:
            ...
            logger.error(f"创建Qdrant客户端失败: {str(e)}")
            raise VectorStoreException(f"创建Qdrant客户端失败: {str(e)}")
    
    def create_or_update_index(self, book_id: str, documents: List[Document]) -> VectorStoreIndex:
        """创建或更新索引"""
        try:
            if self.vector_store_type == "chroma":
                ...
                raise VectorStoreException(f"不支持的向量存储类型: {self.vector_store_type}")
        except Exception as e:
            logger.error(f"创建或更新索引失败: {str(e)}")
            raise VectorStoreException(f"创建或更新索引失败: {str(e)}")
    
    def _create_or_update_chroma_index(self, book_id: str, documents: List[Document]) -> VectorStoreIndex:
        """创建或更新ChromaDB索引"""
        try:
            # 获取ChromaDB客户端
            ...
            logger.info(f"成功创建或更新ChromaDB索引: {book_id}, 文档数量: {len(documents)}")
            return index
        except Exception as e:
            logger.error(f"创建或更新ChromaDB索引失败: {str(e)}")
            raise VectorStoreException(f"创建或更新ChromaDB索引失败: {str(e)}")
    
    def _create_or_update_qdrant_index(self, book_id: str, documents: List[Document]) -> VectorStoreIndex:
        """创建或更新Qdrant索引"""
        try:
            # 获取Qdrant客户端
            ...
            logger.info(f"成功创建或更新Qdrant索引: {book_id}, 文档数量: {len(documents)}")
            return index
        except Exception as e:
            logger.error(f"创建或更新Qdrant索引失败: {str(e)}")
            raise VectorStoreException(f"创建或更新Qdrant索引失败: {str(e)}")
    
    def get_index(self, book_id: str) -> Optional[VectorStoreIndex]:
        """获取索引"""
        try:
            if self.vector_store_type == "chroma":
    ...
    def _get_chroma_index(self, book_id: str) -> Optional[VectorStoreIndex]:
        """获取ChromaDB索引"""
        try:
            # 获取ChromaDB客户端
            chroma_client = self._get_chroma_client(book_id)
            ...
            logger.error(f"获取ChromaDB索引失败: {str(e)}")
            return None
    
    def _get_qdrant_index(self, book_id: str) -> Optional[VectorStoreIndex]:
        """获取Qdrant索引"""
        try:
            # 获取Qdrant客户端
            client = self._get_qdrant_client()
            ...
            logger.error(f"获取Qdrant索引失败: {str(e)}")
            return None
    ...
    def _delete_chroma_index(self, book_id: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"删除ChromaDB集合失败(可能不存在): {str(e)}")
                return False
        except Exception as e:
            logger.error(f"删除ChromaDB索引失败: {str(e)}")
            return False
    ...
    def _delete_qdrant_index(self, book_id: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"删除Qdrant集合失败(可能不存在): {str(e)}")
                return False
        except Exception as e:
            logger.error(f"删除Qdrant索引失败: {str(e)}")
            return False
    ...
    def check_index_exists(self, book_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"检查索引是否存在失败: {str(e)}")
            return False
